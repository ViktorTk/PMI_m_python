...
"""
# Ищем среднее значение столбца 'Literacy (%)', группируем по столбцу 'Region'
avg_per_region = df.groupby('Region')['Literacy (%)'].mean()
...
# loc = фильтр
df.loc[
    df['Literacy (%)_x'].isna() # выбор пустых значений из df
    , 'Literacy (%)_x' # выбор столбца, куда буду записывать значение
] = df.loc[
    df['Literacy (%)_x'].isna() # выбрали также пустые значения из df
    , 'Literacy (%)_y' # столбец из которого брал значения для df => среднее значение по региону
]
...
"""
df['all_GDP'] = (df['Population'] * df['GDP ($ per capita)']) / 1000000

print(df[['Country', 'Population', 'GDP ($ per capita)', 'all_GDP', 'Literacy (%)']])

print(df[df['Literacy (%)'].isna()]['Country'])
# print(df['Literacy (%)'].to_list())
"""
